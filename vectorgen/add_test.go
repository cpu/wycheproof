...
// TestAddIntoGroup adds a single test into an existing group identified by
// its source name. Only the matched group's tests array grows; the others are
// unchanged and the new test inherits the next sequential tcId.
func TestAddIntoGroup(t *testing.T) {
	...
	// Append one test into the re-encryption group. tcIds 1..9 already exist;
	// the new test should become tcId 10. Use a copy of an existing entry so
	// schema validation passes.
	newTest := []byte(`{"comment": "synthetic into-group test",
		...
		"result": "valid", "flags": ["MalleableCiphertext"]}`)
	env := vectorgen.AddEnvelope{
		IntoGroup: "github/lukaszobernig/reenc",
		Tests:     []jsontext.Value{jsontext.Value(newTest)},
	}
	...
	// Quick checks: numberOfTests bumped from 9 to 10, tcId 10 appears.
	if !bytes.Contains(got, []byte(`"numberOfTests": 10,`)) {
		t.Errorf("expected numberOfTests bumped to 10")
	}
	if !bytes.Contains(got, []byte(`"tcId": 10,`)) {
		t.Errorf("expected tcId 10 to be assigned")
	}
	if !bytes.Contains(got, []byte(`"synthetic into-group test"`)) {
		t.Errorf("expected new test comment to appear")
	}
	if bytes.Count(got, []byte(`"source":`)) != 2 {
		t.Errorf("expected exactly 2 sources (no new group added), got %d", bytes.Count(got, []byte(`"source":`)))
	}
}
