...
Fixture for `TestUpdateAddsFields`. Recreates the *contents* added by the
first commit of [PR #254](https://github.com/C2SP/wycheproof/pull/254): the
`ek` field on every test plus the `K` field on tcId 1.
...
- `mlkem_semi_expanded_decaps_test_schema.json`, `common.json` — schemas at
  commit `f0b4b70`, needed because the embedded schemas predate the `ek`/`K`
  additions.
